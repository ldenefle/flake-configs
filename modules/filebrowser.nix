...
let
  inherit (lib)
    generators types hasPrefix literalExpression mkEnableOption mkIf mkOption
    mkPackageOption optional optionalAttrs;
  ...
  configFile = pkgs.writeText "filebrowser-config.json" (generators.toJSON { }
    ({
      ...
      // (optionalAttrs cfg.disablePreviewResize {
        disable-preview-resize = true;
      }) // (optionalAttrs cfg.disableCommandRunner { disable-exec = true; })
      // (optionalAttrs cfg.disableTypeDetectionByHeader {
...
in {
  options.services.filebrowser = {
    ...
    disableTypeDetectionByHeader =
      mkEnableOption "Disable file type detection by reading file headers.";
  ...
  config = mkIf cfg.enable {
    systemd.services.filebrowser = {
      ...
      serviceConfig = {
        ...
        BindPaths = optional (!(hasPrefix "/var/lib/filebrowser" cfg.rootDir))
          cfg.rootDir;
  ...
  meta.maintainers = with lib.maintainers; [ ldenefle ];
}
